...
class DegreeCalculator:
    ...
    def process_tweet_data(self, hashtags, timestamp):
        while self.tweets_heap and not DegreeCalculator.tweets_belong_to_the_same_window(self.tweets_heap[0][0], timestamp):
    ...
    def get_average_degree(self):
        ...
        return 0 if n_vertices == 0 else sum / float(n_vertices)
...
with open(input_path) as input_file, open(output_path, 'w') as output_file:
    for line in input_file:
        ...
        if u"entities" not in data: #or not data[u"entities"][u"hashtags"] or not data[u"created_at"]:
            continue
